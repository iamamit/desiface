    ...
    <div className="space-y-2">
      <div className="bg-white dark:bg-[#1c1c1c] rounded-lg border border-[#E0DFDC] dark:border-[#2E2E2E] p-4">
        <div className="flex justify-between items-center mb-4">
          ...
          <Link href="/connections" className="text-xs text-[var(--accent)] hover:underline font-semibold">
            See all
          </Link>
        </div>
          ...
          {suggestions.map((u) => (
            <div key={u.id} className="flex items-start gap-3">
              <Link href={`/profile/${u.username}`}>
                <div className="w-12 h-12 rounded-full bg-[var(--accent-light)] dark:bg-[var(--accent-light-dark)] flex items-center justify-center text-[var(--accent)] font-bold text-sm flex-shrink-0">
